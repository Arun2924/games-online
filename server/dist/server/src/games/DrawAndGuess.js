...
exports.DrawAndGuess = void 0;
const words_1 = require("./drawAndGuess/words");
exports.DrawAndGuess = {
    id: 'draw-guess',
    minPlayers: 2,
    maxPlayers: 6,
    createState: (players) => {
    ...
    handleCustomEvent: (room, eventData, socket, io) => {
        if (!room.gameState || room.gameState.state.phase !== 'playing')
            return;
        if (socket.id !== room.gameState.state.drawerId)
        ...
        if (eventData.type === 'draw' || eventData.type === 'clear' || eventData.type === 'undo') {
            if (eventData.type === 'clear') {
                ...
                if (eventData.strokeId) {
                    room.gameState.state.drawHistory = room.gameState.state.drawHistory.filter((stroke) => stroke.strokeId !== eventData.strokeId);
                }
            }
            else {
...
function transitionToResults(state) {
    return {
        ...state,
        state: {
            ...state.state,
